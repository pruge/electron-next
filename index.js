...
const adjustRenderer = (directory, _paths) => {
  const paths = ['/_next', '/static'].concat(_paths);
...
module.exports = async (directories, port, paths = []) => {
  if (Array.isArray(port)) {
    paths = Array.from(port);
    port = undefined;
  }
  ...
  if (!isDev) {
    adjustRenderer(directories.production, paths);
    return;
  }
